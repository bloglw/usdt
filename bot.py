...
import config
from database import add_user, get_user, update_balance, init_db
...
async def handler(event):
    ...
    if text == '/start':
        ...
        await handle_balance(event, user_id)
    elif text == '/withdraw':
...
async def show_main_menu(event):
    buttons = [
        [Button.inline("充值地址", b"deposit")],
        [Button.inline("查询余额", b"balance")],
        [Button.inline("提币申请", b"withdraw")]
    ]
    await event.respond('欢迎使用USDT钱包机器人！', buttons=buttons)
...
async def handle_balance(event, user_id):
    user = get_user(user_id)
    address = user[1] if user else None
    if address:
        balance = get_latest_balance(address)
        update_balance(user_id, balance)
        await event.respond(f'您的账户余额是：{balance} TRC20 USDT')
    else:
        await event.respond('未找到您的充值地址，请先获取充值地址。')
...
async def handle_withdraw_initiate(event, user_id):
    user = get_user(user_id)
    balance = float(user[3]) if user else 0  # 将余额转换为浮点数
    if balance < 11:
...
async def handle_withdraw_all(event, user_id):
    ...
    balance = float(user[3]) if user else 0  # 将余额转换为浮点数
    if balance >= 11:
        withdraw_requests[user_id]['amount'] = balance - 1
        await event.respond('请输入您的提币地址：')
    else:
        await event.respond('最低提币金额为10 USDT + 1 USDT手续费，请保证账户余额不低于11 USDT。')
...
async def handle_withdraw_confirm(event, user_id):
    if user_id in withdraw_requests:
        ...
        balance = float(user[3]) if user else 0  # 将余额转换为浮点数
        if balance >= (amount + 1):
            update_balance(user_id, balance - (amount + 1))
